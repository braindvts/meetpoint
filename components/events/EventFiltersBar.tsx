...
const field =
  "w-full rounded-xl border border-accent/20 bg-ink/60 px-3 py-2.5 text-sm text-ivory outline-none transition placeholder:text-muted/60 focus:border-accent/45";

export default function EventFiltersBar({ value, onChange }: Props) {
  const set = (patch: Partial<EventFilters>) => onChange({ ...value, ...patch });

  return (
    <div className="space-y-3 rounded-2xl border border-accent/15 bg-[#12110f]/80 p-3 sm:p-4">
