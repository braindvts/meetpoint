...
type Props = {
  chatId: string;
  /** Fill the parent pane instead of a full-page shell. */
  embedded?: boolean;
  /** Mobile / embedded back control. */
  onBack?: () => void;
};

export default function ChatThreadPanel({ chatId, embedded = false, onBack }: Props) {
  ...
  return (
    <>
      <div className={`mp-chat-shell ${embedded ? "is-embedded" : ""}`}>
        <div
          className={`mx-auto flex h-full min-h-0 w-full flex-col ${
            embedded ? "max-w-none px-3 sm:px-4" : "max-w-3xl px-3 sm:px-6"
          }`}
        >
          <header className="shrink-0 border-b border-line/60 pb-2.5 pt-2 sm:py-3">
            <div className="flex items-center gap-3">
              {onBack ? (
                <button
                  type="button"
                  onClick={onBack}
                  className="shrink-0 text-[11px] font-semibold uppercase tracking-[0.2em] text-muted transition hover:text-accent-2 lg:hidden"
                >
                  ← Back
                </button>
              ) : (
