...
import { CONCLAVE_TABLES } from "./events";
import {
  expandTopicTokens,
  inferLookingFor,
  inferRoles,
...
export function extractEventSignals(event: MeetEvent): EventSignals {
  const blob = [
    event.title,
    event.description,
    event.topics.join(" "),
  ...
  const tokens = bagFrom(event.title, 2.6);
  mergeBags(tokens, bagFrom(event.description, 1.3));
  mergeBags(tokens, bagFrom(event.topics.join(" "), 2.2));
  mergeBags(tokens, bagFrom(event.audience.join(" "), 1.8));
  mergeBags(tokens, bagFrom(event.hostRole, 1.4));
  mergeBags(tokens, bagFrom((event.keywords || []).join(" "), 1.6));
  for (const tag of event.topics) {
...
export function buildIdf(events: MeetEvent[]): Map<string, number> {
...
function topicScore(member: MemberSignals, event: MeetEvent): { score: number; overlap: string[] } {
  if (!member.interests.length) {
    // Sparse: mine tokens against event topics / aliases.
    const hits: string[] = [];
    for (const topic of event.topics) {
      const aliases = expandTopicTokens(topic);
      if (aliases.some((a) => (member.tokens.get(a) || 0) > 0 || (a.length > 3 && member.tokens.has(a)))) {
        hits.push(topic);
      }
    }
  ...
  for (const interest of member.interests) {
    if (event.topics.includes(interest) && !seen.has(interest)) {
      seen.add(interest);
    ...
    const relatedHit = event.topics.find((t) => related.includes(t));
    if (relatedHit && !seen.has(relatedHit)) {
      seen.add(relatedHit);
      raw += 0.34;
      continue;
    }
    const soft = event.topics.find((t) => topicAliasesHit(interest, t));
...
function intentScore(
  ...
  const overlap = event.audience.filter((a) => mine.includes(a));
  const specific = overlap.filter((a) => !GENERIC_INTENTS.has(a));
  const generic = overlap.filter((a) => GENERIC_INTENTS.has(a));
  let score =
    (specific.length * 1 + generic.length * 0.35) / Math.max(event.audience.length, 1);

  const phraseHay = `${member.phrases.join(" ")} ${member.blob}`.toLowerCase();
  const purposeHay = `${event.title} ${event.description} ${(event.keywords || []).join(" ")}`.toLowerCase();
  let phraseHits = 0;
  ...
  if (phraseHay.includes("design career") && purposeHay.includes("design")) phraseHits += 1;
  if (phraseHits) score = Math.min(1, score + 0.18 * Math.min(phraseHits, 2));

  return { score: Math.min(1, score), overlap };
}

function roleScore(member: MemberSignals, event: MeetEvent): { score: number; overlap: string[] } {
  const overlap = event.roles.filter((r) => member.roles.includes(r));
  if (overlap.length) {
    return { score: Math.min(1, 0.55 + overlap.length * 0.22), overlap };
  }
  const job = (member.blob || "").toLowerCase();
  const soft = event.roles.filter((r) => job.includes(r));
  if (soft.length) return { score: 0.36, overlap: soft };
  return { score: 0, overlap: [] };
}

function similarByTopics(a: MeetEvent, b: MeetEvent): boolean {
  if (a.id === b.id) return false;
  const shared = a.topics.filter((t) => b.topics.includes(t));
  return shared.length >= 2 || (shared.length === 1 && a.audience.some((x) => b.audience.includes(x)));
}
...
function feedbackAdjust(
  ...
  for (const row of interests) {
    ...
    if (!other || !similarByTopics(event, other)) continue;
    if (row.status === "passed") delta -= 7;
    if (row.status === "going" || row.status === "saved") {
      delta += 6;
      reason = {
        kind: "feedback",
        label: row.status === "going" ? "Like a table you're going to" : "Like a table you saved",
        weight: 6,
      };
    }
  }
...
export function scoreEvent(
  ...
  const catalog = opts?.catalog || CONCLAVE_TABLES;
  const idf = opts?.idf || buildIdf(catalog);

  const topics = topicScore(member, event);
  const intent = intentScore(member, event);
  const roles = roleScore(member, event);
  const text = cosine(member.tokens, ev.tokens, idf);

  const distance = me.city ? distanceKm(me.city, event.city) : 9999;
  const isLocal = Number.isFinite(distance) && distance <= LOCAL_RADIUS_KM;
  const sameCountry = !!me.city && me.city.country === event.city.country;
  const geo = isLocal ? 1 : sameCountry ? 0.4 : 0.08;

  const specificIntent = intent.overlap.some((a) => !GENERIC_INTENTS.has(a));
  const roleWeight = topics.score >= 0.2 || specificIntent || member.sparse ? 18 : 7;
  ...
  // Precision: a single weak echo should not surface.
  const hasCore =
...
export function isStrongEventMatch(m: EventMatchResult): boolean {
  ...
  // They told us what they're into — require a real topic hit, not just "founder" + Partnership.
  if (m.topicOverlap.length > 0) return true;
  const specific = m.intentOverlap.some((a) => !GENERIC_INTENTS.has(a));
  return specific && m.textScore >= 0.24;
}

/**
 * Rank tables for a member. Precision-first: weak matches are dropped,
 * not padded to fill a grid.
 */
export function rankEvents(
  me: MemberMatchInput,
  events: MeetEvent[] = CONCLAVE_TABLES,
  interests: EventInterest[] = []
): EventMatchResult[] {
...
export function tableKindLabel(kind: MeetEvent["kind"]): string {
