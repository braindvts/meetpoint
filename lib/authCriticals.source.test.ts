...
test("C1 does not recreate a privileged owner unless the server gate is on", () => {
  const src = readFileSync(join(ROOT, "app/api/auth/email/route.ts"), "utf8");
  assert.match(src, /matchesWalkthroughOwner/);
  assert.match(src, /provisionWalkthroughOwnerIfAbsent/);
